...
Interim demo path: `docs/phase-0-simulation-plan.md` defines a Docker-based simulation lane so the deployment model can be demonstrated and the remaining adapters can be developed without waiting for real Phase 0 answers. That simulation does not mark Phase 0 tasks as confirmed; real cutover work is tracked in `docs/phase-0-real-cutover.md`. Sim Phase 1 (local persistence: `adapterMode`, `FileSystemDeployHistoryRepository`, `deployctl status` reading simulated state) and Sim Phase 2 (backend container: Docker app-server, `scripts/ec2/deploy-backend.sh`, `DockerSimSsmDeployExecutor`, `deployctl deploy backend` running end to end against it) are done; see the simulation plan for remaining Sim Phases (frontend artifacts, logs, production replacement demo, runbook).
