...
Interim demo path: `docs/phase-0-simulation-plan.md` defines a Docker-based simulation lane so the deployment model can be demonstrated and the remaining adapters can be developed without waiting for real Phase 0 answers. That simulation does not mark Phase 0 tasks as confirmed; real cutover work is tracked in `docs/phase-0-real-cutover.md`. Sim Phase 1 (local persistence: `adapterMode`, `FileSystemDeployHistoryRepository`, `deployctl status` reading simulated state) is done; see the simulation plan for remaining Sim Phases (Docker app-server container, frontend artifacts, logs, production replacement demo, runbook).
